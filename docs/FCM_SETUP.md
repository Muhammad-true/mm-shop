...
#### Для веб-версии админ панели (автоматическая регистрация)

При входе в админ панель система автоматически:
1. Запрашивает разрешение на уведомления у пользователя
2. Генерирует/получает FCM токен
3. Регистрирует токен на сервере

**Примечание:** Для production рекомендуется использовать реальный FCM токен от Firebase. Текущая реализация использует упрощенный подход для демонстрации.

#### Для мобильных приложений (Flutter/React Native)

Необходимо зарегистрировать токен устройства вручную:

```javascript
// Пример для Web/Flutter
...
- Для веб-версии админ панели регистрация токена происходит автоматически при входе
- Service Worker (`firebase-messaging-sw.js`) обрабатывает push-уведомления в фоновом режиме
- Для production рекомендуется настроить Firebase проект и использовать реальные FCM токены

## Настройка для production (веб-версия)

Для полноценной работы push-уведомлений в веб-версии:

1. Создайте Firebase проект и получите конфигурацию
2. Добавьте конфигурацию в `admin/config.js`:
```javascript
FCM: {
...
3. Обновите `admin/js/fcm.js` для использования Firebase SDK вместо упрощенной версии
