...
3. **Создайте Upload Preset:**
   ...
   - **Трансформации:** (вкладка "Incoming Transformation")
     ...
     **Вариант 2: С удалением фона (платная функция):**
     ```
     ...
     ⚠️ **ВАЖНО:** 
     - Добавьте `fl_auto` в первую трансформацию для автоматической обработки EXIF ориентации
     - Цепочка трансформаций разделяется символом `/`
     - `e_background_removal:fineedges_y` - удаление фона с тонкими краями (платная функция)
     - `b_rgb:D5FFD1` - светло-зеленый фон (#D5FFD1), можно заменить на `b_white` для белого
     ...
     **Расшифровка сложной цепочки:**
     ...
     - `/f_jpg` - формат JPG
     - `/e_background_removal:fineedges_y` - удаление фона с тонкими краями
