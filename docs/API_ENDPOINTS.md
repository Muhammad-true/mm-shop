...
## 🖥️ POS эндпоинты для владельцев магазинов (синхронизация со складом и продажами)

### Массовая загрузка товаров со склада

#### `POST /api/v1/shop/pos/products/bulk-upload` или `POST /api/v1/pos/products/bulk-upload`
Массовая загрузка товаров со склада. Создает новые товары или обновляет существующие (по названию и бренду, или по штрих-коду для вариаций).
      ...
      "variations": [
        {
          ...
          "imageUrls": [],
          "imageUrlsByColor": {
            "Красный": ["/images/variations/red1.jpg", "/images/variations/red2.jpg"],
            "Синий": ["/images/variations/blue1.jpg", "/images/variations/blue2.jpg"]
          }
        }
      ]
    }
...
```json
{
  ...
  "updated": 0,
  "failed": 0,
  "productIds": ["uuid-товара"]
}
```

**Логика работы:**
- Если товар с таким названием и брендом не существует - создается новый
- Если товар существует - обновляется (название, описание, бренд, категория)
- Вариации ищутся по штрих-коду или по размерам/цветам
- Если вариация найдена - обновляется количество со склада (`stockQuantity`)
- Если вариация не найдена - создается новая

---

### Синхронизация продаж

#### `POST /api/v1/shop/pos/sales/sync` или `POST /api/v1/pos/sales/sync`
Синхронизация продаж из POS программы. Уменьшает количество товаров на складе после продажи в магазине.
...
- Уменьшает `stockQuantity` на количество проданных единиц
- Если количество становится 0, устанавливает `isAvailable = false`
- Возвращает информацию об обновленных количествах

---

### Получение информации о количестве товаров

#### `GET /api/v1/shop/pos/products/stock` или `GET /api/v1/pos/products/stock`
Получить информацию о количестве товаров на складе.
...
**Параметры запроса:**
- `barcode` (string, опционально) - фильтр по штрих-коду
- `sku` (string, опционально) - фильтр по SKU
- `in_stock` (boolean, опционально) - только товары с количеством > 0
...
#### `PUT /api/v1/shop/pos/products/:variationId/stock` или `PUT /api/v1/pos/products/:variationId/stock`
Обновить количество конкретной вариации товара.
